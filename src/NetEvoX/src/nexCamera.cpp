...
nexCamera::nexCamera(nexAgedLayout *layout)
{
	reset();
...
void nexCamera::setClippingPlane()
{
	zNear = 1.0f;
...
ofxVec3f nexCamera::getEye()
{
	return eye;
}

ofxVec3f nexCamera::getCenter()
{
	return center;
}

ofxVec3f nexCamera::convertToLocal(ofxVec3f _v)
{
	return convertToLocal(_v.x, _v.y, _v.z);
}

ofxVec3f nexCamera::convertToLocal(float _x, float _y, float _z)
{
...
void nexCamera::tumble(float _px, float _py, float _x, float _y)
{
...
ofxVec3f nexCamera::compArcBallVector(float _x, float _y)
{
	ofxVec3f v;
...
void nexCamera::track(float _px, float _py, float _x, float _y)
{
	float dx = _px - _x;
	float dy = _py - _y;
...
void nexCamera::dolly(float _px, float _py, float _x, float _y)
{
	float dx = _px - _x;
	float dy = _py - _y;
...
void nexCamera::mousePressed(ofMouseEventArgs& event) 
{
...
void nexCamera::mouseDragged(ofMouseEventArgs& event) 
{
	if(event.button == 0)
	{
...
void nexCamera::mouseReleased(ofMouseEventArgs& event) 
{

}

void nexCamera::windowResized(ofResizeEventArgs& event)
{
	setPerspective();
}
...
float nexCamera::getMinX(){
	int i;
	float minX = 0.0f;
	MSA::Physics::Particle* p;
